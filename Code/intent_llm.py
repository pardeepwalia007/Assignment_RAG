...
class QuerySpec(BaseModel):
    intent: str = Field(description="The action: 'ranking', 'aggregation','filter', or distribution,or metadata ")
...
class QueryInterpreter:
    ...
    def _normalize_spec(self, spec: QuerySpec) -> QuerySpec:
        ...
        if intent == "distribution" and spec.entity_columns and not spec.metric:
            # Distribution usually means frequency distribution of a category
            spec.intent = "ranking"
            spec.metric = "COUNT(*)"
            spec.refined_instruction = f'Count records by "{spec.entity_columns[0]}" and sort descending.'
            intent = "ranking"
            spec.entity_columns = [spec.entity_columns[0]]
    ...
    # ----------------------------
    def refine_intent(self, user_query: str, business_context: str = None) -> QuerySpec:
        """
        ...
        system_template = """You are a Support Assistant. Map the user query to the schema using the provided Business Context.
        ...
        2. ENTITY SELECTION: Return a LIST of columns to group by.
        ...
        4. DO NOT answer the user. ONLY return JSON that matches the QuerySpec schema exactly. No extra keys.
        {format_instructions}
        """
