...
def format_doc_evidence(doc_text: str, n_chars: int = 4000) -> str:
    doc_text = (doc_text or "").strip()
    if not doc_text:
        return "STATUS: NO_DOC_EVIDENCE_FOUND"
    return "DOC_EVIDENCE:\n" + doc_text[:n_chars]
...
def summarize_with_llama(question: str, evidence: dict, source_type: str,):
    ...
    sql_block = format_rows(headers, rows)
    is_truncated = "RESULT_TRUNCATED: TRUE" in sql_block
    notes_line = (
        "NOTES_LINE: Text output is truncated for readability; aggregation is calculated over all matching records."
        if is_truncated else
        "NOTES_LINE: Aggregation is calculated over all matching records."
    )
    ...
    doc_block = format_doc_evidence(rules_full)

    # risks_only = extract_risks(rules_full)

    has_sql_rows = isinstance(sql_out, dict) and sql_out.get("sql_result") and (sql_out["sql_result"].get("rows") or [])
    has_doc = "STATUS: NO_DOC_EVIDENCE_FOUND" not in doc_block

    if source_type.lower().startswith("docs"):
        observed_data = doc_block
    elif (not has_sql_rows) and has_doc:
        # Hybrid or SQL-only fallback: prefer docs if SQL returned nothing
        observed_data = doc_block
    else:
        observed_data = sql_observed_data
...
4) If <OBSERVED_DATA> contains "STATUS: NO_QUANTITATIVE_DATA_FOUND" OR "STATUS: NO_DOC_EVIDENCE_FOUND":
     
5) Start immediately with 'EXECUTIVE SUMMARY'.
     
6) Do NOT introduce rankings unless explicit.

7) CITATION RULE:
- If <OBSERVED_DATA> contains a line that starts with "Reference:",
  ALWAYS include that exact line as the final line of the response.
...
10) If INTENT == 'distribution', NEVER use time-based language (e.g., monthly, daily, trend, over time).   
...
D. Docs-Only Policy Answers:
    - If <OBSERVED_DATA> starts with "DOC_EVIDENCE:", answer in 2–5 bullets using only sentences copied from DOC_EVIDENCE.
    - Then add the final line by copying the "Reference: ..." line verbatim if present.
